...
def get_nutrition_info(food_name, quantity, unit, measurement_type):
    ...
    print("inside get_nutrition_info")
    try:
        # Check ChromaDB for existing data
        ...
        print(f"Searching ChromaDB with food_id: {food_id}")
        ...
        try:
            ...
            # Check if we got results and the first result is similar enough
            if (results and 
                results['documents'] and 
                len(results['documents']) > 0):
                
                # Get the distance (similarity score)
                distance = float(results['distances'][0][0])  # Access first element of the distances list
                print(f"Similarity search distance: {distance}")
                
                if distance < 0.5:  # Lower distance means more similar
                    unit_nutrition_data = json.loads(results['documents'][0][0])  # Access first element
...
def record_meal():
    if request.method == 'POST':
        ...
        try:
            ...
            unit_nutrition_data = {
                'calories': safe_float(request.args.get('calories')),
                'protein': safe_float(request.args.get('protein')),
                'fat': safe_float(request.args.get('fat')),
                'carbs': safe_float(request.args.get('carbs')),
                'base_quantity': safe_float(request.args.get('base_quantity')),
                'base_unit': request.args.get('base_unit')  # Keep as string
            }
